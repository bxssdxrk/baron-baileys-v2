...
var RateLimiter = class {
  ...
  async getDelay(recipient, content) {
    ...
    if (hourMessages.length >= this.config.maxPerHour) {
      ...
      const delay2 = oldestInHour ? oldestInHour.timestamp + TIME_CONSTANTS.MS_PER_HOUR - now : TIME_CONSTANTS.MS_PER_HOUR;
    ...
    if (minuteMessages.length >= this.config.maxPerMinute) {
      ...
      const delay2 = oldestInMinute ? oldestInMinute.timestamp + TIME_CONSTANTS.MS_PER_MINUTE - now : TIME_CONSTANTS.MS_PER_MINUTE;
...
var PresenceChoreographer = class {
  ...
  shouldMarkRead() {
    ...
    if (this.config.circadian.enabled) {
      const circadianMultiplier = getCircadianMultiplier(/* @__PURE__ */ new Date(), this.config.circadian.profile, this.config.circadian.timezone);
  ...
  computeTypingPlan(messageLength) {
    ...
    if (this.config.circadian.enabled) {
      circadianMultiplier = getCircadianMultiplier(/* @__PURE__ */ new Date(), this.config.circadian.profile, this.config.circadian.timezone);
    }
...
var PostReconnectThrottle = class {
  ...
  getStats() {
    ...
    const remainingMs = isThrottled && this.throttledSince ? Math.max(0, this.config.rampDurationMs - (Date.now() - this.throttledSince)) : 0;
    return {
      isThrottled,
...
function isLegacyConfig(cfg) {
  if (typeof cfg !== "object" || cfg === null)
    return false;
  return "rateLimiter" in cfg || "warmUp" in cfg || "health" in cfg || "timelock" in cfg || "replyRatio" in cfg || "contactGraph" in cfg || "presence" in cfg || "retryTracker" in cfg || "reconnectThrottle" in cfg || "lidResolver" in cfg || "jidCanonicalizer" in cfg || "sessionStability" in cfg;
}
function mapLegacyToFlat(legacy) {
  console.warn("[baileys-antiban] DEPRECATED: Nested config (v2 style) detected. Migrate to flat config: new AntiBan({ maxPerMinute: 8 }). See: https://github.com/kobie3717/baileys-antiban#migration");
  const flat = {};
...
var AntiBan = class {
  ...
  constructor(input, warmUpStateArg) {
    ...
    if (legacyPassthrough?.sessionStability?.enabled) {
      const healthConfig = {
        ...
        onDegraded: (stats) => {
          if (this.logging) {
            console.log(`[baileys-antiban] \u{1F534} SESSION DEGRADED \u2014 Bad MAC rate: ${stats.badMacCount} in last ${legacyPassthrough?.sessionStability?.badMacWindowMs || 6e4}ms`);
  ...
  async beforeSend(recipient, content) {
    ...
    if (!reconnectThrottleDecision.allowed) {
      this.stats.messagesBlocked++;
      if (this.logging) {
        console.log(`[baileys-antiban] \u{1F504} BLOCKED \u2014 reconnect throttle: ${reconnectThrottleDecision.reason}`);
      }
      return {
        allowed: false,
    ...
    if (this.resolvedConfig.groupProfiles && shouldUseGroupProfile(recipient)) {
      ...
      if (stats.lastMinute >= groupLimits.maxPerMinute || stats.lastHour >= groupLimits.maxPerHour || stats.lastDay >= groupLimits.maxPerDay) {
        this.stats.messagesBlocked++;
        if (this.logging) {
    ...
    if (distractionCheck.pause) {
      ...
      if (this.logging) {
        console.log(`[baileys-antiban] \u23F8\uFE0F  Distraction pause: +${Math.floor(distractionCheck.durationMs / 6e4)}min`);
      }
    }
...
function credsSnapshot(config) {
  ...
  async function list() {
    ...
    try {
      ...
      const snapshots = await Promise.all(files.filter((f) => f.startsWith("creds-") && f.endsWith(".json")).map(async (f) => {
    ...
    const wrappedReadMessages = async (keys) => {
      ...
      const oldMessages = keys.every((key) => {
        if (!key.messageTimestamp)
          return false;
        const msgTime = typeof key.messageTimestamp === "number" ? key.messageTimestamp * 1e3 : parseInt(key.messageTimestamp, 10) * 1e3;
